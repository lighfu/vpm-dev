Shader "Hidden/ltspass_arlocked_ltspass_transparent_f452608b"
{
    Properties
    {
    ...
    SubShader
    {
        HLSLINCLUDE
            ...
            // shader 側で K_runtime_packed と XOR することで:
            //   K_runtime = K_correct のとき: effective_seed = encryption_seed → 復号成立 (元画像復元)
            //   K_runtime = 0 (locked)     のとき: effective_seed = encryption_seed XOR K_correct → 完全 noise
            ...
            // sRGB ↔ linear 変換 (Unity の LinearToGammaSpace / GammaToLinearSpace の代替)。
            // HLSLINCLUDE は Pass HLSLPROGRAM に prepend されるが、 lilToon の include で Unity headers が
            // 引かれる前に我々の関数が parse される → Unity macro が undeclared で compile error になる。
            // pow(x, 2.2) / pow(x, 1/2.2) の近似で Unity 依存を排除する (sRGB 標準関数とは小さな差があるが
            // byte 量子化の誤差が dominant なので XOR round-trip には十分な精度)。
            float3 _AR_LinearToSrgb(float3 c) { return pow(saturate(c), 1.0/2.2); }
            ...
            float3 _AR_DecodePixelRGB(float3 col, float2 uv, uint storedSeed, float2 texSizeWH)
            {
                ...
                // ── sRGB 色空間に整合させる重要処理 ──
                // CPU 側 (Color32) は sRGB byte 空間で XOR している。
                // Unity の Linear color space project では sRGB texture sample 時に
                // 自動で sRGB→linear 変換が走り、 col は linear value で来る。
                // そのまま byte 化すると CPU の sRGB byte と XOR 空間がズレるので、
                // _AR_LinearToSrgb で sRGB byte 空間に戻してから XOR、 decode 後
                // _AR_SrgbToLinear で linear に戻して lilToon の lighting 計算に渡す。
                float3 col_srgb = _AR_LinearToSrgb(col);
                uint3 col_int = (uint3)(round(col_srgb * 255.0));
            ...
            // OVERRIDE_MAIN: lilToon の `_MainTex` sample 直後 (lighting 前) に decode を inject。
            // lighting / emission / matcap は decode 後の色に対して計算されるため視覚的完全互換。
            #define OVERRIDE_MAIN \
                ...
                { \
                    ...
                    if (_ar_seed_main != 0u) \
                    { \
                        fd.col.rgb = _AR_DecodePixelRGB(fd.col.rgb, fd.uvMain, _ar_seed_main, _MainTex_TexelSize.zw); \
                    } \
                } \
