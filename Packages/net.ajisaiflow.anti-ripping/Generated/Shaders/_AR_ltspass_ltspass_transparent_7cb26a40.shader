Shader "Hidden/ltspass_arlocked_ltspass_transparent_7cb26a40"
{
    Properties
    {
        ...
        // ── AjisaiAR injection (v0.31: texture pixel encryption seeds) ──
        ...
        // ── AjisaiAR injection (v0.31: XOR Sort + Mapping mode 用 texture) ──
        // 値があれば XorSortMapping mode、 default white (= 全 0 indices = identity ish) は他 mode で無視される
    ...
    SubShader
    {
        HLSLINCLUDE
            ...
            // ── v0.31: texture pixel encryption (XOR PRNG / LCG) ──
            // CPU 側 (TextureXorEncryptor.Encrypt) と完全に同じ式で各 pixel に XOR mask を適用する。
            // material._AR_MainTexSeedLo/Hi に焼き込まれた値は (encryption_seed XOR K_correct_packed)。
            // shader 側で TK_runtime_packed と XOR することで:
            //   TK_runtime = K_correct のとき: effective_seed = encryption_seed → 復号成立 (元画像復元)
            //   TK_runtime = 0 (locked)     のとき: effective_seed = encryption_seed XOR K_correct → 完全 noise
            // seed = 0 (= 暗号化されていない material) のときは gate で decode を skip。
            //
            ...
            // Unity が _MainTex のサイズを自動 populate する magic uniform (lilToon は declare していないので我々が declare)。
            // float4(1/width, 1/height, width, height)。 .zw が (width, height)。
            float4 _MainTex_TexelSize;
            // v0.31 XorSortMapping mode 用 mapping texture (RGBA32 Point filter で per-block 16 byte forwardMap 格納)。
            // HLSL モードでは Texture2D + SamplerState 形式で宣言する必要あり (sampler2D + tex2Dlod は HLSL モードで failing)。
            // lilToon の TEXTURE2D macro は lil_common_macro.hlsl で定義されるが我々の HLSLINCLUDE はそれより前に parse されるため raw HLSL 宣言を使う。
            Texture2D _AR_MainTexMapping;
            SamplerState sampler_AR_MainTexMapping;
            float4 _AR_MainTexMapping_TexelSize;
            // sRGB → linear 変換 (Unity の GammaToLinearSpace の代替)。
            // HLSLINCLUDE は Pass HLSLPROGRAM に prepend されるが、 lilToon の include で Unity headers が
            // 引かれる前に我々の関数が parse される → Unity macro は undeclared で compile error になる。
            // pow(x, 2.2) の近似で Unity 依存を排除する (sRGB 標準関数とは小さな差があるが
            // 視覚的にはほぼ識別不能、 lighting 計算前の入力値変換のみに使うため lilToon 出力色には影響微小)。
            ...
            {
                // texture decode 専用 key (_AR_TK0..3) を使う。 vertex decode 用 _AR_K0..3 は
                // SMR で 0 に固定されてしまうため texture decode には使えない。
                uint key_packed = ((uint)_AR_TK0)
                                | (((uint)_AR_TK1) << 8)
                                | (((uint)_AR_TK2) << 16)
                                | (((uint)_AR_TK3) << 24);
                uint effective_seed = storedSeed ^ key_packed;
                uint2 pix = (uint2)(uv * texSizeWH);
                uint idx = pix.y * (uint)texSizeWH.x + pix.x;
                // Knuth multiplicative hash で seed と idx を mix → LCG 1 ステップ
                uint s = effective_seed + idx * 2654435761u;
                s = s * 1664525u + 1013904223u;
                // ── encrypted texture は linear (= sRGB 変換無し) で保存されているので
                //   col = encrypted_byte / 255 (生値) として届く。
                //   round で逆量子化すれば encrypted_byte を完全復元 → XOR で original sRGB byte 復元 ──
                uint3 col_int = (uint3)(round(saturate(col) * 255.0));
                uint3 mask_int = uint3(s & 0xFFu, (s >> 8) & 0xFFu, (s >> 16) & 0xFFu);
                float3 decoded_srgb = float3(col_int ^ mask_int) * (1.0/255.0);
                // 元 sRGB byte / 255 を lilToon が期待する linear 色値に変換して返す。
                // この pow(2.2) 近似誤差は byte XOR 後の単一変換なので decode 自体には影響しない。
                return _AR_SrgbToLinear(decoded_srgb);
            }
            // OVERRIDE_MAIN (XorSortMapping mode, 16×16 region sort): mapping texture から sortedIdx 取り出し encrypted を再 sample → XOR decode
            // 完全 noise (safety mode 耐性) + BC7 圧縮 + 解錠時クリーン復元の理想的トレードオフ。
            // 16×16 region 単位で sort することで BC7 4×4 block 内の pixel 値が tightly clustered になり量子化誤差が大幅減少。
            #define OVERRIDE_MAIN \
                LIL_GET_MAIN_TEX \
                { \
                    uint _ar_seed_main = (((uint)_AR_MainTexSeedHi) << 16) | ((uint)_AR_MainTexSeedLo); \
                    if (_ar_seed_main != 0u) \
                    { \
                        uint _ar_kp = ((uint)_AR_TK0) | (((uint)_AR_TK1) << 8) | (((uint)_AR_TK2) << 16) | (((uint)_AR_TK3) << 24); \
                        uint _ar_es = _ar_seed_main ^ _ar_kp; \
                        float2 _ar_size = _MainTex_TexelSize.zw; \
                        uint2 _ar_pix = (uint2)(fd.uvMain * _ar_size); \
                        uint2 _ar_region = _ar_pix / 16u; \
                        uint2 _ar_local = _ar_pix - _ar_region * 16u; /* in [0,16) */ \
                        uint _ar_li = _ar_local.y * 16u + _ar_local.x; /* in [0,256) */ \
                        /* mapping texel: localIdx/4 = texel index in 8×8 region grid */ \
                        uint _ar_t_idx = _ar_li >> 2; /* [0,64) */ \
                        uint _ar_ch = _ar_li & 3u; /* RGBA channel */ \
                        uint2 _ar_t_local = uint2(_ar_t_idx & 7u, _ar_t_idx >> 3); /* texel pos in 8×8 grid */ \
                        uint2 _ar_mp = _ar_region * 8u + _ar_t_local; \
                        float2 _ar_mu = (float2(_ar_mp) + 0.5) * _AR_MainTexMapping_TexelSize.xy; \
                        float4 _ar_msmp = LIL_SAMPLE_2D_LOD(_AR_MainTexMapping, sampler_AR_MainTexMapping, _ar_mu, 0); \
                        uint4 _ar_mb = uint4(round(saturate(_ar_msmp) * 255.0)); \
                        uint _ar_sorted = _ar_ch == 0u ? _ar_mb.r : (_ar_ch == 1u ? _ar_mb.g : (_ar_ch == 2u ? _ar_mb.b : _ar_mb.a)); \
                        /* sorted is in [0,256), region 内 raster 配置: x = sorted % 16, y = sorted / 16 */ \
                        uint2 _ar_sl = uint2(_ar_sorted & 15u, _ar_sorted >> 4); \
                        uint2 _ar_sp = _ar_region * 16u + _ar_sl; \
                        float2 _ar_su = (float2(_ar_sp) + 0.5) * _MainTex_TexelSize.xy; \
                        float4 _ar_smp = LIL_SAMPLE_2D_LOD(_MainTex, sampler_MainTex, _ar_su, 0); \
                        /* XOR decode: per-pixel mask は元の global_idx (= _ar_pix.y * width + _ar_pix.x) ベース */ \
                        uint _ar_gi = _ar_pix.y * (uint)_ar_size.x + _ar_pix.x; \
                        uint _ar_s = _ar_es + _ar_gi * 2654435761u; \
                        _ar_s = _ar_s * 1664525u + 1013904223u; \
                        uint3 _ar_ci = (uint3)(round(saturate(_ar_smp.rgb) * 255.0)); \
                        uint3 _ar_mk = uint3(_ar_s & 0xFFu, (_ar_s >> 8) & 0xFFu, (_ar_s >> 16) & 0xFFu); \
                        float3 _ar_dec_srgb = float3(_ar_ci ^ _ar_mk) * (1.0/255.0); \
