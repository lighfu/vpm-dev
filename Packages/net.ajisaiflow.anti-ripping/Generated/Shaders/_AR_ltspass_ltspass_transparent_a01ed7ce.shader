Shader "Hidden/ltspass_arlocked_ltspass_transparent_a01ed7ce"
{
    Properties
    {
    ...
    SubShader
    {
        HLSLINCLUDE
            ...
            float3 _AR_DecodePixelRGB(float3 col, float2 uv, uint storedSeed, float2 texSizeWH)
            {
                ...
                // LinearToGammaSpace で sRGB byte 空間に戻してから XOR、 decode 後
                // GammaToLinearSpace で linear に戻して lilToon の lighting 計算に渡す。
                // (Gamma color space project では LinearToGammaSpace が identity なので無害)
                float3 col_srgb = LinearToGammaSpace(saturate(col));
                ...
                return GammaToLinearSpace(decoded_srgb);
            }
