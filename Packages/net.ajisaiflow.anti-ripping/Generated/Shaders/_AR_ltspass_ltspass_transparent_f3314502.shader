Shader "Hidden/ltspass_arlocked_ltspass_transparent_f3314502"
{
    Properties
    {
    ...
    SubShader
    {
        HLSLINCLUDE
            ...
            float3 _AR_DecodePixelRGB(float3 col, float2 uv, uint storedSeed, float2 texSizeWH)
            {
                ...
                //   round で逆量子化 → modular 減算 (mod 256) で original sRGB byte 復元 ──
                //   注: v0.31.x で XOR から modular 減算に変更。 BC7 量子化誤差 ±N が XOR では
                //       元 byte の bit pattern により増幅 (例: BC7 +2 → decoded ±255) されるが、
                //       modular 減算なら ±N がそのまま decoded ±N となり増幅されない。
                uint3 col_int = (uint3)(round(saturate(col) * 255.0));
                uint3 mask_int = uint3(s & 0xFFu, (s >> 8) & 0xFFu, (s >> 16) & 0xFFu);
                uint3 decoded_int = (col_int - mask_int) & 0xFFu;
            ...
            // OVERRIDE_MAIN (XorSortMapping mode, ★ Global sort): mapping から (sx, sy) 取得 → encrypted sample → XOR decode
            // texture 全体が luminance 順に top→bottom 並び替え、 boundary noise なしで BC7 圧縮対応。
            // 完全 noise (safety mode 耐性) + 解錠時クリーン復元 + 元解像度維持。
            #define OVERRIDE_MAIN \
                ...
                { \
                    ...
                    { \
                        ...
                        /* mapping texture sample at fragment uv (mapping は元解像度と同じ size)。 Point filter で正確な byte 取得 */ \
                        ...
                        /* (sx, sy) を 16-bit packed RGBA から復元: R=sx_high, G=sx_low, B=sy_high, A=sy_low */ \
                        uint _ar_sx = (_ar_mb.r << 8) | _ar_mb.g; \
                        uint _ar_sy = (_ar_mb.b << 8) | _ar_mb.a; \
                        /* encrypted texture sample at sorted position */ \
                        ...
                        /* Modular subtract decode (mod 256): BC7 量子化誤差 ±N が増幅されない */ \
                        /* per-pixel mask は元の global_idx (= _ar_pix.y * width + _ar_pix.x) ベース */ \
                        ...
                        uint3 _ar_dec_int = (_ar_ci - _ar_mk) & 0xFFu; \
