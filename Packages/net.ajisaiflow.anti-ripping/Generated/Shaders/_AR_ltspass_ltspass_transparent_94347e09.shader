Shader "Hidden/ltspass_arlocked_ltspass_transparent_94347e09"
{
    Properties
    {
    ...
    SubShader
    {
        HLSLINCLUDE
            ...
            float3 _AR_DecodePixelRGB(float3 col, float2 uv, uint storedSeed, float2 texSizeWH)
            {
                ...
                //   round で逆量子化 → XOR で original sRGB byte 復元 ──
                //   注: v0.31.x で modular 減算 → XOR に revert。 modular は wrap-around で
                //       byte 境界 (~6% pixel) に ±250 級 spike noise (salt-and-pepper) が出て
                //       perceptually XOR の chromatic noise (film-grain) より悪化したため。
            ...
            #define OVERRIDE_MAIN \
                ...
                { \
                    ...
                    { \
                        ...
                        /* XOR decode: BC7 ε は film-grain ノイズに留まる (modular の wrap spike を回避) */ \
