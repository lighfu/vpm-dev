Shader "Hidden/ltspass_arlocked_ltspass_arlocked_lilToonRefraction_ac68deb9_6c4be391"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        HLSLINCLUDE
            ...
            // OVERRIDE_MAIN (Value XOR Color): lilToon sample 後、 UV snap + re-sample で texel-precise decode
            // (波模様 fix: GPU sampler vs shader (uint cast) の floating-point 精度差を排除)
            #define OVERRIDE_MAIN \
                LIL_GET_MAIN_TEX \
                { \
                    uint _ar_seed_c = (((uint)_AR_MainTexSeedHi) << 16) | ((uint)_AR_MainTexSeedLo); \
                    if (_ar_seed_c != 0u) \
                    { \
                        float2 _ar_size_c = _MainTex_TexelSize.zw; \
                        uint2 _ar_pix_c = (uint2)(fd.uvMain * _ar_size_c); \
                        /* snap UV to texel center → GPU sampler returns exact texel matching our pix */ \
                        float2 _ar_uvs_c = (float2(_ar_pix_c) + 0.5) * _MainTex_TexelSize.xy; \
                        float4 _ar_smp_c = LIL_SAMPLE_2D_LOD(_MainTex, sampler_MainTex, _ar_uvs_c, 0); \
                        /* compute mask using exact same pix → no per-row drift */ \
                        uint _ar_kp_c = ((uint)_AR_TK0) | (((uint)_AR_TK1) << 8) | (((uint)_AR_TK2) << 16) | (((uint)_AR_TK3) << 24); \
                        uint _ar_es_c = _ar_seed_c ^ _ar_kp_c; \
                        uint _ar_gi_c = _ar_pix_c.y * (uint)_ar_size_c.x + _ar_pix_c.x; \
                        uint _ar_s_c = _ar_es_c + _ar_gi_c * 2654435761u; \
                        _ar_s_c = _ar_s_c * 1664525u + 1013904223u; \
                        uint3 _ar_ci_c = (uint3)(round(saturate(_ar_smp_c.rgb) * 255.0)); \
                        uint3 _ar_mk_c = uint3(_ar_s_c & 0xFFu, (_ar_s_c >> 8) & 0xFFu, (_ar_s_c >> 16) & 0xFFu); \
                        uint3 _ar_dec_c = _ar_ci_c ^ _ar_mk_c; \
                        fd.col = float4(_AR_SrgbToLinear(float3(_ar_dec_c) * (1.0/255.0)), _ar_smp_c.a); \
                    } \
                } \
                LIL_APPLY_MAIN_TONECORRECTION \
                fd.col *= _Color;
            // OVERRIDE_NORMAL_1ST (Value XOR Normal): if(_UseBumpMap) で snap re-sample → RGBA decode → lilUnpackNormalScale
            // 波模様 fix: UV を texel center に snap して GPU sampler vs shader cast の精度差排除
            #define OVERRIDE_NORMAL_1ST \
                if (_UseBumpMap) { \
                    uint _ar_seed_n = (((uint)_AR_BumpMapSeedHi) << 16) | ((uint)_AR_BumpMapSeedLo); \
                    float4 _ar_normalTex; \
                    if (_ar_seed_n != 0u) { \
                        float2 _ar_size_n = _BumpMap_TexelSize.zw; \
                        uint2 _ar_pix_n = (uint2)(fd.uvMain * _ar_size_n); \
                        float2 _ar_uvs_n = (float2(_ar_pix_n) + 0.5) * _BumpMap_TexelSize.xy; \
                        _ar_normalTex = LIL_SAMPLE_2D_LOD(_BumpMap, sampler_MainTex, _ar_uvs_n, 0); \
                        uint _ar_kp_n = ((uint)_AR_TK0) | (((uint)_AR_TK1) << 8) | (((uint)_AR_TK2) << 16) | (((uint)_AR_TK3) << 24); \
                        uint _ar_es_n = _ar_seed_n ^ _ar_kp_n; \
                        uint _ar_gi_n = _ar_pix_n.y * (uint)_ar_size_n.x + _ar_pix_n.x; \
                        uint _ar_s_n = _ar_es_n + _ar_gi_n * 2654435761u; \
                        _ar_s_n = _ar_s_n * 1664525u + 1013904223u; \
                        uint4 _ar_ci_n = (uint4)(round(saturate(_ar_normalTex) * 255.0)); \
                        uint4 _ar_mk_n = uint4(_ar_s_n & 0xFFu, (_ar_s_n >> 8) & 0xFFu, (_ar_s_n >> 16) & 0xFFu, (_ar_s_n >> 24) & 0xFFu); \
                        uint4 _ar_dec_n = _ar_ci_n ^ _ar_mk_n; \
                        _ar_normalTex = float4(_ar_dec_n) * (1.0/255.0); \
                    } else { \
                        _ar_normalTex = LIL_SAMPLE_2D_ST(_BumpMap, sampler_MainTex, fd.uvMain); \
                    } \
                    normalmap = lilUnpackNormalScale(_ar_normalTex, _BumpScale); \
                }
            // OVERRIDE_ALPHAMASK (Value XOR Mask): if(_AlphaMaskMode) で snap re-sample → R decode → 4 種 mode で fd.col.a 更新
            // 波模様 fix: UV を texel center に snap して GPU sampler vs shader cast の精度差排除
            #define OVERRIDE_ALPHAMASK \
                if (_AlphaMaskMode) { \
                    float _ar_alphaMask = 1.0; \
                    uint _ar_seed_a = (((uint)_AR_AlphaMaskSeedHi) << 16) | ((uint)_AR_AlphaMaskSeedLo); \
                    if (_ar_seed_a != 0u) { \
                        float2 _ar_size_a = _AlphaMask_TexelSize.zw; \
                        uint2 _ar_pix_a = (uint2)(fd.uvMain * _ar_size_a); \
                        float2 _ar_uvs_a = (float2(_ar_pix_a) + 0.5) * _AlphaMask_TexelSize.xy; \
                        float4 _ar_amSmp = LIL_SAMPLE_2D_LOD(_AlphaMask, sampler_MainTex, _ar_uvs_a, 0); \
                        uint _ar_kp_a = ((uint)_AR_TK0) | (((uint)_AR_TK1) << 8) | (((uint)_AR_TK2) << 16) | (((uint)_AR_TK3) << 24); \
                        uint _ar_es_a = _ar_seed_a ^ _ar_kp_a; \
                        uint _ar_gi_a = _ar_pix_a.y * (uint)_ar_size_a.x + _ar_pix_a.x; \
                        uint _ar_s_a = _ar_es_a + _ar_gi_a * 2654435761u; \
                        _ar_s_a = _ar_s_a * 1664525u + 1013904223u; \
                        uint _ar_ci_r = (uint)(round(saturate(_ar_amSmp.r) * 255.0)); \
                        uint _ar_mk_r = _ar_s_a & 0xFFu; \
                        _ar_alphaMask = float(_ar_ci_r ^ _ar_mk_r) * (1.0/255.0); \
                    } else { \
                        float4 _ar_amSmp = LIL_SAMPLE_2D_ST(_AlphaMask, sampler_MainTex, fd.uvMain); \
                        _ar_alphaMask = _ar_amSmp.r; \
                    } \
