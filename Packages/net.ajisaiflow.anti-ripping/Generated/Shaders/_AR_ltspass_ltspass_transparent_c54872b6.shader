Shader "Hidden/ltspass_arlocked_ltspass_transparent_c54872b6"
{
    Properties
    {
    ...
    SubShader
    {
        HLSLINCLUDE
            ...
            // OVERRIDE_MAIN (Permutation mode): 4×4 block 内位置を XOR で permutation し permuted UV で再 sample。
            // initial LIL_GET_MAIN_TEX は捨てる (但し POM や _MainTex_ScrollRotate は維持)。
            // pixel 値は変えないので BC7 lossy 圧縮でも decode 後ノイズなし。
            // sRGB → linear 変換は Value XOR mode と同じく shader 側で行う必要あり。
            #define OVERRIDE_MAIN \
                ...
                { \
                    ...
                    { \
                        ...
                        uint _ar_bx = (uint)_ar_size.x / 4u; \
                        uint _ar_bi = _ar_block.y * _ar_bx + _ar_block.x; \
                        uint _ar_s = _ar_es + _ar_bi * 2654435761u; \
                        _ar_s = _ar_s * 1664525u + 1013904223u; \
                        uint _ar_pm = _ar_s & 0xFu; \
                        uint _ar_si = _ar_li ^ _ar_pm; \
                        uint2 _ar_sl = uint2(_ar_si & 3u, _ar_si >> 2); \
                        uint2 _ar_sp = _ar_block * 4u + _ar_sl; \
                        float2 _ar_su = (float2(_ar_sp) + 0.5) * _MainTex_TexelSize.xy; \
                        float4 _ar_smp = LIL_SAMPLE_2D(_MainTex, sampler_MainTex, _ar_su); \
                        fd.col = float4(_AR_SrgbToLinear(_ar_smp.rgb), _ar_smp.a); \
                    } \
                } \
