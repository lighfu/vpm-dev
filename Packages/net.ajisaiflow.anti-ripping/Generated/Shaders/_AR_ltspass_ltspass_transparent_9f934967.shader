Shader "Hidden/ltspass_arlocked_ltspass_transparent_9f934967"
{
    Properties
    {
    ...
    SubShader
    {
        HLSLINCLUDE
            ...
            // OVERRIDE_MAIN (XorSortMapping mode): mapping texture から sortedIdx 取り出し encrypted を再 sample → XOR decode
            ...
            #define OVERRIDE_MAIN \
                ...
                { \
                    ...
                    { \
                        ...
                        uint2 _ar_block = _ar_pix / 4u; \
                        uint2 _ar_local = _ar_pix - _ar_block * 4u; \
                        uint _ar_li = _ar_local.y * 4u + _ar_local.x; \
                        /* mapping texture: per-block 4 RGBA32 texels (= 16 bytes), texel x = block.x*4 + t (t=0..3) */ \
                        uint _ar_byte_idx = _ar_li; /* mapping[origLocalIdx] gives sortedIdx */ \
                        uint _ar_t = _ar_byte_idx >> 2; /* which of 4 texels */ \
                        uint _ar_ch = _ar_byte_idx & 3u; /* which of 4 channels */ \
                        uint2 _ar_mp = uint2(_ar_block.x * 4u + _ar_t, _ar_block.y); \
                        float2 _ar_mu = (float2(_ar_mp) + 0.5) * _AR_MainTexMapping_TexelSize.xy; \
                        /* lilToon の LIL_SAMPLE_2D_LOD は CG/HLSL 両モード対応 (HLSL モードでは tex.SampleLevel(samp, uv, lod) に展開される) */ \
                        ...
                        uint2 _ar_sl = uint2(_ar_sorted & 3u, _ar_sorted >> 2); \
                        uint2 _ar_sp = _ar_block * 4u + _ar_sl; \
